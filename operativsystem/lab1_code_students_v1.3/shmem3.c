...
// The consumer tends to bottleneck the producer, by using 10 semaphores each,
...
int main(int argc, char **argv)
{
    ...
    // reduced to 9 whilst semid2 is increased to 10 during the first call in parent thread
        ...
        while (var1 < 100)
        {
            ...
            while (found == 0)
            {
                ...
                if (shmp->empty[i % 10] != 1)
                {
                    ...
                    int t = 100000 * (rand() % (RAND_MAX + 1) + 2);
                    i = i % 10;
                    usleep(t);
