...
int FS::cp(std::string sourcepath, std::string destpath)
{
    ...
    src_name = path_info1.file_name; // stores the information for ease of use
    ...
    if (dest_index == -1) // This happens if no folder is found with said name,
    {                     // then it is assumed that we want to create a new file with said name
        char found = 0;
        for (size_t i = 0; i < BLOCK_SIZE / 64; i++) // looks for an empty slot in the dest directory and
        {
